...
public class GameScreen extends ScreenAdapter {
    ...
    List<Step> mSteps;
    List<Star> mStars;
    List<Enemy> mEnemies; // 敵オブジェ
    Ufo mUfo;
    Player mPlayer;
    Enemy mEnemy;

    float mHeightSoFar; // プレイヤーが地面からどれだけ離れたか
    Vector3 mTouchPoint; // タッチイベントを保持

    BitmapFont mFont; // Bitmapフォント
    int mScore; // スコア
    int mHightScore; // ハイスコア
    Preferences mPrefs; // ハイスコア保存用
    ...
    public GameScreen(JumpActionGame game) {
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.tomohiro.isobe.jumpactiongame");
        mHightScore = mPrefs.getInteger("HIGHTSCORE", 0);
    ...
    public void render(float delta) {
        ...
        // カメラの中心を超えたらカメラを上に移動させる　つまり画面の上半分には絶対行かない
    ...
    private void createStage() {
        ...
        Texture enemyTexture = new Texture("dragon.png");
    ...
    private void checkCollision() {
        ...
        for (int i = 0; i < mStars.size(); i++) {
            ...
            if (mPlayer.getBoundingRectangle().overlaps(star.getBoundingRectangle())) {
                ...
                if (mScore > mHightScore) {
                    mHightScore = mScore; // ハイスコアの更新
                    // ハイスコアをPreferenceに保存する
                    mPrefs.putInteger("HIGHTSCORE", mHightScore);
                    mPrefs.flush();
                }
                break;
            }
        ...
        for (int i = 0; i < mEnemies.size(); i++) {
            ...
            if (mPlayer.getBoundingRectangle().overlaps(enemy.getBoundingRectangle())) {
                Sound sound = Gdx.audio.newSound(Gdx.files.internal("data/explosion06.wav"));
                sound.play(1.0f);
